...
class Sky(Canvas):
    def __init__(self, master, parameters_file):
        ...
        self.__simulation = Simulation(
            _NUM_BOIDS, _CANVAS_SIZE, _CANVAS_SIZE, parameters_file
        )
        self.__boids = {}

        for id in self.__simulation.ids():
            self.__boids[id] = self.create_rectangle(
                0, 0, 0 + _BOID_SIZE, 0 + _BOID_SIZE, fill="red"
            )
    ...
    def __refresh(self):
        ...
        for id, pos in positions.items():
            self.coords(
                self.__boids[id], pos.x, pos.y, pos.x + _BOID_SIZE, pos.y + _BOID_SIZE
            )
